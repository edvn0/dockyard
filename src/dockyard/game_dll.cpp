...
auto GameDll::load(const VFSPath path)
  ...
  std::filesystem::copy_file(resolved, dll->loaded_path,
  ...
  if (dll->handle == nullptr) {
    ...
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, GetLastError(),
                   MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), msg, sizeof(msg),
                   nullptr);
    return std::unexpected(std::format("Failed to load game DLL ({}): {}",
                                       dll->loaded_path.string(), msg));
#else
    return std::unexpected(std::format("Failed to load game DLL ({}): {}",
                                       dll->loaded_path.string(), dlerror()));
#endif
  }
  ...
  info("GameDll: loaded '{}' (gen {})", dll->source_path.filename().string(),
       dll->generation);
  return dll;
}
...
auto GameDll::start_watching(BS::priority_thread_pool &pool) -> void {
  ...
  pool.detach_task([this, alive = watching] { poll_task(alive); },
                   BS::pr::lowest);
}
...
auto GameDll::force_reload() -> bool { return do_reload(); }
...
auto GameDll::poll_task(std::shared_ptr<std::atomic<bool>> alive) -> void {
  ...
  if (alive->load() && (thread_pool != nullptr)) {
    thread_pool->detach_task(
        [this, alive = std::move(alive)] { poll_task(std::move(alive)); },
        BS::pr::lowest);
  }
}
...
auto GameDll::make_loaded_path() const -> std::filesystem::path {
  return source_path.parent_path() /
         (source_path.stem().string() + "_hot_" + std::to_string(generation) +
...
auto GameDll::do_reload() -> bool {
  ...
  std::error_code ec{};
  std::filesystem::copy_file(source_path, loaded_path,
                             std::filesystem::copy_options::overwrite_existing,
                             ec);
  ...
  auto *factory =
      reinterpret_cast<GameFactory>(platform_symbol(new_handle, "create_game"));
  ...
  if (ec) {
    dy::warn("GameDll: could not remove previous hot DLL '{}': {}",
             old_loaded_path.string(), ec.message());
  }
