...
#else
  void *p = mmap(nullptr, reserve_bytes, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS,
                 -1, 0);
  mem.base = (p == MAP_FAILED) ? nullptr : p;
#endif

  assert(mem.base != nullptr &&
         "Failed to reserve virtual memory for GameMemory");
  return mem;
}
...
auto GameMemory::reset() -> void { used = 0; }

constexpr auto round_up_to_alignment = [](std::unsigned_integral auto v,
                                          auto align_size) {
...
#ifdef _WIN32
  VirtualAlloc(static_cast<char *>(base) + committed, new_committed - committed,
               MEM_COMMIT, PAGE_READWRITE);
#else
  mprotect(static_cast<char *>(base) + committed, new_committed - committed,
