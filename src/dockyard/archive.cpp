...
static constexpr size_t kCommitChunk = 2ULL << 20; // 2 MiB commit granularity

auto vm_reserve(size_t bytes) -> void* {
#ifdef _WIN32
...
auto vm_ensure_committed(void* base, size_t already_committed, size_t up_to)
    -> std::expected<size_t, std::string>
{
#ifdef _WIN32
    if (up_to <= already_committed) return already_committed;
    size_t new_commit = ((up_to + kCommitChunk - 1) / kCommitChunk) * kCommitChunk;
    // Don't overshoot — caller already validated against reserved
    if (!VirtualAlloc(static_cast<char*>(base) + already_committed,
                      new_commit - already_committed,
                      MEM_COMMIT, PAGE_READWRITE))
        return std::unexpected("VirtualAlloc MEM_COMMIT failed");
    return new_commit;
#else
    (void)base; (void)already_committed; (void)up_to;
    return up_to; // always "committed" on POSIX
#endif
}

auto vm_fit(void* base, size_t reserved, size_t used) -> void {
    size_t used_pages = (used + kPageSize - 1) & ~(kPageSize - 1);
    if (used_pages >= reserved) return;
#ifdef _WIN32
    VirtualFree(static_cast<char*>(base) + used_pages,
                reserved - used_pages, MEM_DECOMMIT);
#else
    madvise(static_cast<char*>(base) + used_pages,
            reserved - used_pages, MADV_DONTNEED);
#endif
}

auto vm_release(void* base, size_t reserved) -> void {
    if (!base) return;
#ifdef _WIN32
    (void)reserved;
...
auto MemoryBundle::at(std::string_view key) const -> std::span<const std::byte> {
...
struct BundleWriter {
    ...
    auto ensure(size_t size) -> std::expected<void, std::string> {
        const size_t needed = b.used + size;
    ...
    auto commit_entry(std::string name, size_t size) -> void {
...
struct EntryWriter {
    ...
    auto finish_entry(std::string name, size_t size) -> void {
...
auto parse_octal(const char* s, size_t n) -> uint64_t {
    uint64_t v = 0;
...
struct ByteStream {
    virtual auto read(std::byte* buf, size_t n) -> size_t = 0;
    virtual ~ByteStream()                                  = default;
};

auto drain_blocks(ByteStream& stream, uint64_t padded_size,
                  std::array<std::byte, 512>& block) -> void {
    for (uint64_t d = 0; d < padded_size; d += 512)
        stream.read(block.data(), 512);
}

// dest  — disk output root (used when bw == nullptr)
// bw    — VM bundle writer (used when non-null)
static auto extract_tar_impl(ByteStream& stream, const fs::path& dest, BundleWriter* bw)
    -> std::expected<void, std::string>
{
    ...
    for (;;) {
        ...
        const uint64_t padded   = (size + 511u) & ~uint64_t{511u};
        ...
        if (typeflag == 'L') {
            ...
            while (remaining > 0) {
                ...
                size_t copy = std::min(size_t{512}, remaining);
                std::memcpy(pending_long_name.data() + offset, block.data(), copy);
                offset += copy; remaining -= copy;
            }
        ...
        if (typeflag == '0' || typeflag == '7') {
            ...
            if (bw) {
                if (auto r = bw->ensure(size); !r) return r;
                EntryWriter ew{*bw};
                ...
                while (remaining > 0) {
                    ...
                    size_t n = static_cast<size_t>(std::min(remaining, uint64_t{512}));
                ...
                while (remaining > 0) {
                    if (stream.read(block.data(), 512) < 512)
                        return std::unexpected("truncated tar data");
                    auto n = static_cast<std::streamsize>(
                        std::min(remaining, uint64_t{512}));
...
class GzipStream final : public ByteStream {
    ...
    static constexpr size_t kInBufSize = 65536;
    ...
    auto refill() -> void {
        if (in_pos_ < in_avail_ || src_eof_) return;
        file_.read(reinterpret_cast<char*>(in_buf_), kInBufSize);
        in_avail_ = static_cast<size_t>(file_.gcount());
        in_pos_   = 0;
        if (in_avail_ < kInBufSize) src_eof_ = true;
    }

    auto pump() -> void {
        while (!done_ && avail_ == 0) {
            refill();
            if (in_avail_ == 0) { done_ = true; return; }

            if (dict_ofs_ == kDictSize) dict_ofs_ = 0;

            size_t    in_size  = in_avail_ - in_pos_;
            size_t    out_size = kDictSize - dict_ofs_;
            mz_uint32 flags    = src_eof_ ? 0u : TINFL_FLAG_HAS_MORE_INPUT;

            tinfl_status st = tinfl_decompress(
                &decomp_,
                in_buf_ + in_pos_, &in_size,
                dict_, dict_ + dict_ofs_, &out_size, flags);
            ...
            if (st <= TINFL_STATUS_DONE) done_ = true;
            if (in_size == 0 && out_size == 0 && !done_) { done_ = true; return; }
        }
    }
...
public:
    explicit GzipStream(std::ifstream f) : file_(std::move(f)) { tinfl_init(&decomp_); }
    ...
    auto skip_header() -> std::expected<void, std::string> {
        ...
        if (auto r = expect(0x1f); !r) return r;
        if (auto r = expect(0x8b); !r) return r;
        if (auto r = expect(8);    !r) return r;

        auto flg = read_raw_byte();
        if (!flg) return std::unexpected(flg.error());

        for (int i = 0; i < 6; ++i) // MTIME(4) + XFL + OS
            if (auto r = read_raw_byte(); !r) return std::unexpected(r.error());

        if (*flg & 0x04) {
            auto lo = read_raw_byte(); if (!lo) return std::unexpected(lo.error());
            auto hi = read_raw_byte(); if (!hi) return std::unexpected(hi.error());
            uint16_t xlen = uint16_t(*lo) | uint16_t(uint16_t(*hi) << 8);
            for (uint16_t i = 0; i < xlen; ++i)
                if (auto r = read_raw_byte(); !r) return std::unexpected(r.error());
        }
        for (int f = 0; f < 2; ++f) { // FNAME (0x08), FCOMMENT (0x10)
            if (!(*flg & (0x08u << f))) continue;
            for (;;) {
                auto b = read_raw_byte(); if (!b) return std::unexpected(b.error());
                if (*b == 0) break;
            }
        }
        if (*flg & 0x02) { // FHCRC
    ...
    auto read(std::byte* buf, size_t n) -> size_t override {
        size_t total = 0;
        while (total < n) {
            if (avail_ == 0) { pump(); if (avail_ == 0) break; }
            size_t copy = std::min(avail_, n - total);
...
class ZstdStream final : public ByteStream {
    static constexpr size_t kInBufSize  = 131072;
    static constexpr size_t kOutBufSize = 131072;
    ...
    auto fill_in() -> void {
        if (zin_.pos < zin_.size || src_eof_) return;
        file_.read(reinterpret_cast<char*>(in_buf_), kInBufSize);
        size_t n = static_cast<size_t>(file_.gcount());
        zin_     = {in_buf_, n, 0};
        if (n < kInBufSize || file_.eof()) src_eof_ = true;
    }
...
public:
    ...
    auto read(std::byte* buf, size_t n) -> size_t override {
        ...
        while (total < n && !done_) {
            if (out_avail_ > 0) {
                size_t copy = std::min(out_avail_, n - total);
            ...
            fill_in();
            if (zin_.size == 0) { done_ = true; break; }
            ZSTD_outBuffer zout{out_buf_, kOutBufSize, 0};
            size_t ret = ZSTD_decompressStream(dstream_, &zout, &zin_);
            if (ZSTD_isError(ret)) { done_ = true; break; }
            out_pos_   = 0;
            out_avail_ = zout.pos;
...
static auto extract_zip_impl(const fs::path& src, const fs::path& dest, BundleWriter* bw)
    -> std::expected<void, std::string>
{
    mz_zip_archive zip{};
    if (!mz_zip_reader_init_file(&zip, src.string().c_str(), 0))
        return std::unexpected(std::format("cannot open zip '{}'", src.string()));
    struct Guard { mz_zip_archive* z; ~Guard() { mz_zip_reader_end(z); } } g{&zip};
    ...
    for (mz_uint i = 0; i < count; ++i) {
        ...
        if (bw) {
            if (auto r = bw->ensure(stat.m_uncomp_size); !r) return r;
            EntryWriter ew{*bw};
            auto* dst = ew.begin_entry();
...
auto estimate_zip(const fs::path& src) -> std::optional<uint64_t> {
    mz_zip_archive zip{};
    if (!mz_zip_reader_init_file(&zip, src.string().c_str(), 0))
        return std::nullopt;
    struct Guard { mz_zip_archive* z; ~Guard() { mz_zip_reader_end(z); } } g{&zip};
    ...
    for (mz_uint i = 0; i < n; ++i) {
        ...
        if (!mz_zip_reader_file_stat(&zip, i, &st)) return std::nullopt;
        if (!mz_zip_reader_is_file_a_directory(&zip, i))
...
// Returns nullopt when the encoder omitted the size field (streaming compression).
// For multi-frame zstd this only reads the first frame — use as an upper bound.
auto estimate_zstd(const fs::path& src) -> std::optional<uint64_t> {
    constexpr size_t kReadSize = 256;
...
auto estimate_gzip(const fs::path& src) -> std::optional<uint64_t> {
    std::ifstream f(src, std::ios::binary | std::ios::ate);
    if (!f) return std::nullopt;
    auto file_size = static_cast<uint64_t>(f.tellg());
    if (file_size < 20) return std::nullopt; // minimum valid gzip
    ...
    if (sz == 0) return std::nullopt; // ambiguous: could be 0 mod 2^32
    return static_cast<uint64_t>(sz);
}
...
auto open_file(const fs::path& src) -> std::expected<std::ifstream, std::string> {
    std::ifstream f(src, std::ios::binary);
    if (!f) return std::unexpected(std::format("cannot open '{}'", src.string()));
    return f;
}
    ...
    -> std::expected<void, std::string>
{
    ...
    if (ec)
        return std::unexpected(
            std::format("cannot create dest '{}': {}", dest_dir.string(), ec.message()));
    ...
    if (is_tar_gz) {
        ...
        if (auto r = gz.skip_header(); !r) return r;
    ...
    return std::unexpected(std::format(
        "unsupported archive format '{}' (supported: .zip .tar.gz .tgz .tar.zst .tzst)",
        src.extension().string()));
}

auto extract_to_memory(const dy::VFSPath& src_vfs, size_t budget)
    -> std::expected<MemoryBundle, std::string>
{
    ...
    // --- Cheap size estimate ---
    std::optional<uint64_t> estimated;
    if      (is_zip)     estimated = estimate_zip(src);
    else if (is_tar_zst) estimated = estimate_zstd(src);
    else if (is_tar_gz)  estimated = estimate_gzip(src);
    ...
    const size_t reserve_size = estimated ? *estimated : budget;
    void* base = vm_reserve(reserve_size);
    if (!base)
        return std::unexpected(std::format(
            "failed to reserve {:.1f} GiB of virtual address space",
    ...
    if (is_zip) {
        ...
        auto f = open_file(src);
        if (!f) { vm_release(base, reserve_size); return std::unexpected(f.error()); }
