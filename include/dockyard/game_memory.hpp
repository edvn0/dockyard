...
struct GameMemory {
  ...
  [[nodiscard]] static auto create(usize reserve_bytes = default_reserve)
      -> GameMemory;
  auto destroy() -> void;
  auto reset() -> void;

  template <typename T> [[nodiscard]] auto root() -> T * {
    assert(base && "no allocation yet");
    return static_cast<T *>(base);
  }

  template <typename T> [[nodiscard]] auto allocate() -> T * {
